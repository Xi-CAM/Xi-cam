...
from qtpy.QtWidgets import QLabel, QErrorMessage, QSizePolicy, QPushButton, QHBoxLayout, QVBoxLayout, QComboBox, QWidget, QMenu, QAction, QGridLayout
...
class SliceablePanel(QWidget):
    ...
    def setData(self, data:DataArray):
        self.data = data
        self.slice = {dim: 0 for dim in data.dims[2:]}

        self.full_view.setData(data[self.slice])
        if self.right_view:
            self.right_view.setData(data)
        if self.top_view:
            self.top_view.setData(data)
        if self.corner_view:
            self.corner_view.setData(data)

    def toggleHorizontalSlice(self, enable):
        if enable:
            if not self.right_view:
                self.right_view = SliceablePanel()
                if self.data is not None:
                    # get the coordinate of the vertical slice
                    slice = self.full_view.getCrosshairPos()[1]

                    # set the right view to the same xarray, but with the second coordinate pre-sliced
                    self.right_view.setData(self.data.sel(**{self.data.dims[1]: slice}, method='nearest'))
    ...
    def toggleVerticalSlice(self, enable):
        if enable:
            if not self.top_view:
                self.top_view = SliceablePanel()
                if self.data is not None:
                    # get the coordinate of the horizontal slice
                    slice = self.full_view.getCrosshairPos()[0]

                    # pre-slice the first coord
                    top_data: DataArray = self.data.sel(**{self.data.dims[0]: slice}, method='nearest')

                    # transpose the next two dims (to match visually with the layout)
                    top_data = top_data.transpose(top_data.dims[1], top_data.dims[0], ...)

                    # set the top view to the same xarray, but with the first coordinate pre-sliced
                    self.top_view.setData(top_data)
...
class SliceableGraphicsView(GraphicsView):
    ...
    def __init__(self):
        ...
        self._vline = InfiniteLine((0, 0), angle=90, movable=True, pen=linepen)
        self._hline = InfiniteLine((0, 0), angle=0, movable=True, pen=linepen)
        self.view.addItem(self._vline)
        self.view.addItem(self._hline)
    ...
    def getCrosshairPos(self):
        return self._hline.pos()[1], self._vline.pos()[0]
